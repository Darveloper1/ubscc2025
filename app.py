...
def investigate():
    ...
    items = payload.get("networks")
    if not isinstance(items, list):
        return jsonify(error="Body must contain 'networks': [...]"), 400
    ...
    for item in items:
        ...
        # Build graph (undirected) and keep edge ids
        adj = defaultdict(list)          # node -> list[(neighbor, edge_id)]
        ...
        # Non-bridges = edges on cycles => extra channels
        extra = []
